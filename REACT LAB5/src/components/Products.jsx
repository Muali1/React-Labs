...
const Products = () => {
  ...
  useEffect(() => {
    const getProducts = async () => {
      try {
        setIsLoading(true);
        const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'https://dummyjson.com';
        const skip = (currentPage - 1) * pageSize;
        let url = `${apiBaseUrl}/products?limit=${pageSize}&skip=${skip}`;
        
        if (activeSearchQuery.trim()) {
          url = `${apiBaseUrl}/products/search?q=${encodeURIComponent(activeSearchQuery)}&limit=${pageSize}&skip=${skip}`;
        }
